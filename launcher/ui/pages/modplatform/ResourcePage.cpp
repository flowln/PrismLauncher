...
void ResourcePage::updateUi()
{
    ...
    current_pack.extraData.body.remove(QRegularExpression("<[^>]*(?:center|div)\\W*>"));

    m_ui->packDescription->setHtml(
        text + (current_pack.extraData.body.isEmpty() ? current_pack.description : h.process(current_pack.extraData.body.toUtf8())));
